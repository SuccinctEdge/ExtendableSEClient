...
std::vector<JoinLine> TripleStore::look_up_ele2(ID_TYPE ele0, ID_TYPE ele1){
    ...
    //cout << "0:" << wt_ele0.size() << endl;
    //cout << "1:" << wt_ele1.size() << endl;
    //cout << "2:" << wt_ele2.size() << endl;
...
void TripleStore::store_to_disk(const string &path) {
    ...
    string ts_ele0_path = directory_path + "ts_ele0.st";
    string ts_ele01_bitmap_path = directory_path + "ts_ele01_bitmap.st";
    string ts_ele1_path = directory_path + "ts_ele1.st";
    string ts_ele12_bitmap_path = directory_path + "ts_ele12_bitmap.st";
    string ts_ele2_path = directory_path + "ts_ele2.st";

    //sdsl::store_to_file(wt_ele0, ts_ele0_path);
    //sdsl::store_to_file(wt_ele1_bit_map, ele01_bitmap_path);
    wt_ele1_bit_map->store_to_disk(ts_ele01_bitmap_path);
    //sdsl::store_to_file(wt_ele1, ts_ele1_path);
    //sdsl::store_to_file(wt_ele1_bit_map, ele12_bitmap_path);
    wt_ele2_bit_map->store_to_disk(ts_ele12_bitmap_path);
    //sdsl::store_to_file(wt_ele2, ts_ele2_path);
...
void TripleStore::extend(std::vector<ID_TYPE>& ele0_vector, std::vector<ID_TYPE>& ele1_vector, std::vector<ID_TYPE>& ele2_vector, TripleStoreType ts_type) {
    triple_store_type = ts_type;

    ID_TYPE last_ele0 = -1, last_ele1 = -1;
    int_vector<> ele0_vec(ele0_vector.size()), ele1_vec(ele1_vector.size()), ele2_vec(ele2_vector.size());
    ...
    auto start = clock();
    for(unsigned long long i = 0; i < ele0_vector.size(); ++i){
        ele0_vec[i] = ele0_vector[i];
    }

    for(unsigned long long i = 0; i < ele1_vector.size(); ++i){
        ele1_vec[i] = ele1_vector[i];
    }

    for(unsigned long long i = 0; i < ele2_vector.size(); ++i){
        ele2_vec[i] = ele2_vector[i];
    }
    cout << "  |    -Construct the string of ele0, ele1, ele2 and bit_maps: " << (double)(clock() - start)/CLOCKS_PER_SEC*1000 << endl;

    start = clock();
    wt_ele0.extend(ele0_vec);
    cout << "  |   | Call construct_im to create ele0 wt: " << (double)(clock() - start)/CLOCKS_PER_SEC*1000 << endl;
    start = clock();
    ...
    wt_ele1_bit_map->extend(ele1_bit_vector);

    cout << "  |   | Call construct_im to create ele01 bit map wt: " << (double)(clock() - start)/CLOCKS_PER_SEC*1000 << endl;
    start = clock();
    wt_ele1.extend(ele1_vec);
    cout << "  |   | Call construct_im to create ele1 wt: " << (double)(clock() - start)/CLOCKS_PER_SEC*1000 << endl;
    start = clock();
    //construct_im(wt_ele2_bit_map, ele2_bit_map_str, 'd');
    ...
    wt_ele2_bit_map->extend(ele2_bit_vector);

    cout << "  |   | Call construct_im to create ele12 bit map wt: " << (double)(clock() - start)/CLOCKS_PER_SEC*1000 << endl;
    start = clock();
    wt_ele2.extend(ele2_vec);
    cout << "  |   | Call construct_im to create ele2 wt: " << (double)(clock() - start)/CLOCKS_PER_SEC*1000 << endl;
