...
*/
#ifndef INCLUDED_SDSL_RRR_VECTOR_MOD
#define INCLUDED_SDSL_RRR_VECTOR_MOD
...
class rank_support_rrr_mod;                // in rrr_vector

// forward declaration needed for friend declaration
template<uint8_t t_b=1, uint16_t t_bs=15, class t_rac=int_vector<>, uint16_t t_k=32>
class select_support_rrr_mod;                // in rrr_vector
...
class rrr_vector_mod
{
    ...
    public:
        ...
        typedef random_access_const_iterator<rrr_vector_mod> iterator;
        typedef iterator                                 const_iterator;
        typedef bv_tag                                   index_category;

        typedef rank_support_rrr_mod<1, t_bs, t_rac, t_k>   rank_1_type;
        typedef rank_support_rrr_mod<0, t_bs, t_rac, t_k>   rank_0_type;
        typedef select_support_rrr_mod<1, t_bs, t_rac, t_k> select_1_type;
        typedef select_support_rrr_mod<0, t_bs, t_rac, t_k> select_0_type;

        friend class rank_support_rrr_mod<0, t_bs, t_rac, t_k>;
        friend class rank_support_rrr_mod<1, t_bs, t_rac, t_k>;
        friend class select_support_rrr_mod<0, t_bs, t_rac, t_k>;
        friend class select_support_rrr_mod<1, t_bs, t_rac, t_k>;
    ...
    rrr_vector_mod<t_bs, t_rac, t_k> * select0_vec;
    rrr_vector_mod<t_bs, t_rac, t_k> * select1_vec;
        int_vector<> select_sb0;
        int_vector<> select_sb1;
        // have to be considered as inverted i.e. 1 and
        // 0 are swapped

        void copy(const rrr_vector_mod& rrr)
        {
            m_size = rrr.m_size;
            m_bt = rrr.m_bt;
            m_btnr = rrr.m_btnr;
    ...
    public:
        ...
        //! Copy constructor
        rrr_vector_mod(const rrr_vector_mod& rrr)
        {
            copy(rrr);
        }

        //! Move constructor
        rrr_vector_mod(rrr_vector_mod&& rrr) : m_size(std::move(rrr.m_size)),
    ...
    rrr_vector_mod(const bit_vector& bv, bool selec){
            selectable = selec;
            m_size = bv.size();
            int_vector<> bt_array;
...
//          no extra dummy block added to bt_array, therefore this condition should hold
        ...
        */
        rrr_vector_mod(const bit_vector& bv)
        {
            m_size = bv.size();
            int_vector<> bt_array;

            if(selectable){
                bit_vector set_array(bv.size());
                bit_vector unset_array(bv.size());

                std::map<int, int> select1_map;
                std::map<int, int> select0_map;
                int set_count = 0;
                int unset_count = 0;
                int last_sb0 = 0, last_sb1 = 0;
                int sb_size = t_k * t_bs;

                for (int j = 0; j < bv.size(); j++) {
                    if (bv[j]) {
                        if (j / sb_size != last_sb1) {
                ...
                select1_vec = new rrr_vector_mod<t_bs, t_rac, t_k>(set_array,false);
                unset_array.resize(unset_count);
                select0_vec = new rrr_vector_mod<t_bs, t_rac, t_k>(unset_array,false);

                select_sb0.width(bits::hi(last_sb0)+1);
                select_sb0.resize(select0_map.size()+1);
                select_sb1.width(bits::hi(last_sb1)+1);
                select_sb1.resize(select1_map.size()+1);

                for(int i = 0; i < select0_map.size();i++)
                    select_sb0[i] = select0_map[i];
                for(int i = 0; i < select1_map.size();i++)
                    select_sb1[i] = select1_map[i];
            }
...
//          no extra dummy block added to bt_array, therefore this condition should hold
        ...
        */
        ...
        //! Assignment operator
        rrr_vector_mod& operator=(const rrr_vector_mod& rrr)
        {
            if (this != &rrr) {
                copy(rrr);
            }
        ...
        rrr_vector_mod& operator=(rrr_vector_mod&& rrr)
        {
            swap(rrr);
            return *this;
        }
...
struct rank_support_rrr_mod_trait_mod {
    typedef bit_vector::size_type size_type;
...
template<>
struct rank_support_rrr_mod_trait_mod<0> {
    typedef bit_vector::size_type size_type;
...
class rank_support_rrr_mod
{
        static_assert(t_b == 1u or t_b == 0u , "rank_support_rrr_mod: bit pattern must be `0` or `1`");
    public:
        typedef rrr_vector_mod<t_bs, t_rac, t_k> bit_vector_type;
    ...
    public:
         ...
         */
        explicit rank_support_rrr_mod(const bit_vector_type* v=nullptr)
        {
            set_vector(v);
        }
        ...
        */
        const size_type rank(size_type i)const
        {
            ...
            if (sample_pos+1 < m_v->m_rank.size()) {
                ...
                if (diff_rank == (size_type)0) {
                    return  rank_support_rrr_mod_trait_mod<t_b>::adjust_rank(rank, i);
                } else if (diff_rank == (size_type)t_bs*t_k) {
                    return  rank_support_rrr_mod_trait_mod<t_b>::adjust_rank(
            ...
            return rank_support_rrr_mod_trait_mod<t_b>::adjust_rank(rank + popcnt, i);
        }
        ...
        rank_support_rrr_mod& operator=(const rank_support_rrr_mod& rs)
        {
            if (this != &rs) {
                set_vector(rs.m_v);
            }
        ...
        void swap(rank_support_rrr_mod&) { }
...
class select_support_rrr_mod
{
    ...
    private:
        const bit_vector_type* m_v; //!< Pointer to the rank supported rrr_vector
        rank_support_rrr_mod<1,t_bs, t_rac, t_k> vec_rank;

    /*
        rank_support_rrr_mod<1,t_bs, t_rac, t_k> vec_rank;
        vec_rank.set_vector(m_v->select1_vec);

        size_type idx = m_v->select_sb1[vec_rank.rank(i)];
        size_type begin = idx * t_k;
        size_type rank = m_v->m_rank[begin];
     */
    ...
    size_type select1(size_type i)const {
        ...
        /*
        size_type begin = m_v->select_sb1[vec_rank.rank(i)];
        size_type rank = m_v->m_rank[begin];
        size_type idx = begin * t_k;
        */
    ...
    size_type select0(size_type i)const{
        ...
        size_type sb = m_v->select_sb0[vec_rank.rank(i)];
        size_type rank = (sb*t_bs*t_k) - m_v->m_rank[sb];

        // Calcul du premier index possible dans la bitmap
        size_type begin = sb;
        size_type idx = sb * t_k ;
    ...
    public:
        explicit select_support_rrr_mod(const bit_vector_type* v=nullptr)
        {
            set_vector(v);
        }
        ...
        void set_vector(const bit_vector_type* v=nullptr)
        {
            m_v = v;
            if(v!= nullptr)
                vec_rank=* new rank_support_rrr_mod<1,t_bs, t_rac, t_k>(m_v->select1_vec);
            //vec_rank.set_vector(m_v->select1_vec);
        }

        select_support_rrr_mod& operator=(const select_support_rrr_mod& rs)
        {
            if (this != &rs) {
                set_vector(rs.m_v);
            }
        ...
        void swap(select_support_rrr_mod&) { }
